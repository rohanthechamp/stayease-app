...
const UpdateProfileForm = ({ children, data }: Props) => {
    ...
    const [selectedCountry, setSelectedCountry] = useState<string>(() => {
        return localStorage.getItem("userSelectedCountry") || "";
    });
    ...
    return (
        <div className="max-w-2xl mx-auto">
            {/* Heading */}
            <h2 className="text-2xl font-semibold text-primary-50 mb-2">
                Update your profile
            </h2>
            <p className="text-primary-400 mb-6">
                This helps us speed up your booking experience.
            </p>

            <form
                ref={resetRef}
                className="bg-primary-900/60 backdrop-blur-xl border border-white/10 
        rounded-2xl p-8 flex flex-col gap-6 shadow-xl"
                action={async (formData) => {
                    await handleFormAction(formData);
                    await update();
                    resetRef.current?.reset();
            ...
            >
                ...
                <div className="space-y-2">
                    ...
                    <input
                        value={user?.name ?? ""}
                        disabled
                        className="w-full px-5 py-3 rounded-xl bg-primary-900 
            border border-white/10 text-primary-400 cursor-not-allowed"
                    />
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative">
                        {React.cloneElement(children as React.ReactElement, {
                            ...
                            defaultCountry: selectedCountry,
                            className:
                                "w-full px-5 py-3 rounded-xl bg-primary-950 border border-white/10 text-primary-100 focus:ring-2 focus:ring-accent-500 outline-none",
                        })}
                    </div>
                </div>
            ...
            ${session?.user?.nationalID
                                ...
                                : "bg-primary-950 border-white/10 text-primary-100"
                            }`}
                    />

                    {session?.user?.nationalID && (
                        <p className="text-sm text-green-400">
                            ✓ Identity verified on file
                        </p>
                    )}
                </div>
                ...
                <div className="flex justify-end">
                    <button
                        disabled={pending}
                        className="px-6 py-3 rounded-xl bg-accent-500 text-primary-900 font-semibold 
